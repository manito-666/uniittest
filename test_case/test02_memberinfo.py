...
class MemberTest(unittest.TestCase):
    def setUp(self):
        ...
        self.log.info(message="----------测试开始----------", name="test02_memberinfo.py")

    def tearDown(self):
        self.log.info(message="----------测试结束----------", name="test02_memberinfo.py")

    def test_member(self):
        '''
        验证获取会员信息功能
        '''
        max_row = self.ope.count_case('info', excelPath)
        print(max_row)
        for i in range(max_row-1):
            testdata=self.data.read(["info"])
            method = testdata[i][4]
            ...
            try:
                ...
                self.log.info('测试通过')
                self.ope.write_data('info',i+2,10,con)
                self.ope.write_data('info', i+2, 11,'✅')
            except Exception as e:
                self.log.error('Error:%s' % e)
                self.ope.write_data('info', i+2, 12, '❌')
                self.fail("测试失败")
